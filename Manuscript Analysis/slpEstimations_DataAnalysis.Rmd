...
plot_data <- base::rbind(intRatings_Long1, intRatings_Long2) %>%
  ...
  dplyr::mutate(RatingType = recode_factor(RatingType,
                                           "naive_OT" = "Naïve Listener Orthographic Transcriptions",
                                           "slp_VAS" = "SLP VAS Ratings",
                                           "slp_EST" = "SLP Percent Estimations")) 
...
CorrMatrix <- IntSpeakerRatings %>%
  dplyr::select(!c(SpeakerID, naive_VAS)) %>%
  as.matrix() %>%
  Hmisc::rcorr() 
...
CorrMatrix <- CorrMatrix %>%
  as.data.frame() %>%
  dplyr::mutate(naive_OT = as.numeric(naive_OT),
                slp_EST = as.numeric(slp_EST),
                slp_VAS = as.numeric(slp_VAS)) %>%
  gt::gt() %>%
  gt::fmt_number(columns = c(naive_OT, slp_EST, slp_VAS), decimals = 2) 
...
IntSpeakerRatings_Long <- IntSpeakerRatings %>%
  ...
  dplyr::mutate(Method = fct_relevel(Method, "VAS Rating", "Percent Estimation"))
...
IntSpeakerRatings_Long %>%
  ...
  geom_abline(intercept = 0, slope = 1, linetype = 2) +
  ylab("Naïve Listener Orthographic Transcription Scores") +
  labs(color = "SLP Intelligibility Measures", shape = "SLP Intelligibility Measures") +
